...
class Detector:
    ...
    def setup_psds(self, F_lo=-np.inf, F_hi=np.inf, psd_file_dict=None):
        if psd_file_dict is None or (self.det_key not in list(psd_file_dict.keys()) and self.tec not in list(psd_file_dict.keys())):
            psd_file = None
            is_asd   = None
        elif self.det_key in list(psd_file_dict.keys()):
            psd_file = psd_file_dict[self.det_key]['psd_file']
            is_asd   = psd_file_dict[self.det_key]['is_asd']
        else:
            psd_file = psd_file_dict[self.tec]['psd_file']
            is_asd   = psd_file_dict[self.tec]['is_asd']
    ...
    def calc_det_responses_derivs_num(self, inj_params, deriv_variables, wf, deriv_symbs_string, conv_cos, conv_log, use_rot, step, method, order, n):
        print(' ',self.det_key)
        self.calc_det_responses(wf,inj_params)
        self.del_hf = drd.calc_det_responses_derivs_num(self.loc,wf,deriv_symbs_string,self.f,inj_params,use_rot,'hf',step,method,order,n)
    ...
    ###
    #-----SNR calculations-----
    def calc_snrs(self, only_net, df):
    ...
    def calc_fisher_cov_matrices(self, only_net, df, cond_sup):
        print(' ',self.det_key)
        del_hf_sub_dict = bfs.get_sub_dict(self.del_hf,('hf',),0)
        if not only_net:
            self.fisher, self.cov, self.wc_fisher, self.cond_num = fat.calc_fisher_cov_matrices(list(del_hf_sub_dict.values()),self.psd,self.f,0,df,cond_sup)
            return self.fisher
        else:
            fisher,_,_,_ = fat.calc_fisher_cov_matrices(list(del_hf_sub_dict.values()),self.psd,self.f,1,df,cond_sup)
            return fisher
    ...
    def calc_sky_area_90(self, deriv_variables):
