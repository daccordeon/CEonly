...
(
    ...
    network_id,
    science_case,
    num_injs_per_zbin_per_task,
    total_number_of_files,
    merge_bool,
) = sys.argv[
    1:
]  # first argv is the script's name
task_id, network_id, num_injs_per_zbin_per_task, total_number_of_files, merge_bool = [
    int(x)
    for x in (
        task_id,
        network_id,
...
if merge_bool and (
    total_number_of_files == len(glob.glob(data_path + "results_*_TASK_*"))
):
