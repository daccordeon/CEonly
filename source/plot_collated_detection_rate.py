...
from filename_search_and_manipulation import net_label_styler, net_label_to_network_spec, network_spec_to_net_label, file_name_to_multiline_readable, find_files_given_networks
...
def collate_eff_detrate_vs_redshift(axs, results, zaxis_plot, colours=None, label=None, parallel=True):
    ...
    line_lo, = axs[0].semilogx(zaxis_plot, results.det_eff_fits[0](zaxis_plot), color=colours[0], label=label)
    if colours[1] is None:
        colours[1] = line_lo.get_color()    
    line_hi, = axs[0].semilogx(zaxis_plot, results.det_eff_fits[1](zaxis_plot), color=colours[1], linestyle='--')    
    axs[0].plot(results.zavg_efflo_effhi[:,0], results.zavg_efflo_effhi[:,1], 'o', color=line_lo.get_color(), label=fr'$\rho$ > {SNR_THRESHOLD_LO}')
    axs[0].plot(results.zavg_efflo_effhi[:,0], results.zavg_efflo_effhi[:,2], 's', color=line_hi.get_color(), label=fr'$\rho$ > {SNR_THRESHOLD_HI}')
...
#     plt.legend()
    ...
    axs[1].loglog(zaxis_plot, parallel_map(lambda z : results.det_rate(z, snr_threshold=10),  zaxis_plot, parallel=parallel), color=line_lo.get_color())
    axs[1].loglog(zaxis_plot, parallel_map(lambda z : results.det_rate(z, snr_threshold=100), zaxis_plot, parallel=parallel), color=line_hi.get_color(), linestyle='--')

def compare_detection_rate_of_networks_from_saved_results(network_spec_list, science_case, save_fig=True, show_fig=True, plot_label=None, full_legend=False, specific_wf=None, print_progress=True, data_path='/fred/oz209/jgardner/CEonlyPony/source/data_redshift_snr_errs_sky-area/', parallel=True):
    ...
    for i, file in enumerate(found_files):
        ...
        if i == 0:
            axs[1].loglog(zaxis_plot, parallel_map(results.det_rate_limit, zaxis_plot, parallel=parallel), color='black', linewidth=3, label=f'{results.science_case} merger rate')
#             print(f'maximum detection rate at z={zaxis_plot[-1]} is {det_rate_limit(zaxis_plot[-1])}')
        ...
        collate_eff_detrate_vs_redshift(axs, results, zaxis_plot, label=label, colours=[colour, colour], parallel=parallel)
