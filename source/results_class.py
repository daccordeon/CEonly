...
from filename_search_and_manipulation import network_spec_to_net_label, net_label_styler, filename_to_netspec_sc_wf_injs
...
class InjectionResults(object):
    ...
    def __init__(self, file_name, data_path='/fred/oz209/jgardner/CEonlyPony/source/data_redshift_snr_errs_sky-area/'):
        ...
        self.redshift, self.snr, self.err_logMc, self.err_logDL, self.err_eta, self.err_iota, self.sky_area_90 = self.results.transpose()
        self.network_spec, self.science_case, self.wf_model_name, self.wf_other_var_dic, self.num_injs = filename_to_netspec_sc_wf_injs(self.file_name)
    ...
    def calculate_and_set_detection_rate(self, print_reach=False):
        ...
        self.zmin_plot, self.zmax_plot, num_zbins_fine = 1e-2, 50, 40 # eyeballing 40 bins from Fig 2
        redshift_bins_fine = list(zip(np.geomspace(self.zmin_plot, self.zmax_plot, num_zbins_fine)[:-1],
                                      np.geomspace(self.zmin_plot, self.zmax_plot, num_zbins_fine)[1:])) # redshift_bins are too wide
        self.zavg_efflo_effhi = np.empty((len(redshift_bins_fine), 3))
        for i, (zmin, zmax) in enumerate(redshift_bins_fine):
            z_snr_in_bin = self.results[:,0:2][np.logical_and(zmin < self.redshift, self.redshift < zmax)]
            if len(z_snr_in_bin) == 0:
                self.zavg_efflo_effhi[i] = [np.nan, np.nan, np.nan]
            else:
                self.zavg_efflo_effhi[i,0] = gmean(z_snr_in_bin[:,0]) # geometric mean, just using zmax is cleaner but less accurate
                self.zavg_efflo_effhi[i,1] = np.mean(z_snr_in_bin[:,1] > SNR_THRESHOLD_LO)
                self.zavg_efflo_effhi[i,2] = np.mean(z_snr_in_bin[:,1] > SNR_THRESHOLD_HI)
        ...
        popt_lo, _ = curve_fit(sigmoid_3parameter, self.zavg_efflo_effhi[:,0], self.zavg_efflo_effhi[:,1], method='dogbox', p0=p0, bounds=bounds, maxfev=maxfev)
        if np.all(self.zavg_efflo_effhi[:,2] == 0):
            popt_hi = 1, -1, 1 # f(z) = 0
        else:
            popt_hi, _ = curve_fit(sigmoid_3parameter, self.zavg_efflo_effhi[:,0], self.zavg_efflo_effhi[:,2], method='dogbox', p0=p0, bounds=bounds, maxfev=maxfev)
    ...
    #         perrs = [np.sqrt(np.diag(pcov)) for pcov in pcovs]
        ...
        self.det_eff_fits = [(lambda z : sigmoid_3parameter(z, *popts[0])), (lambda z : sigmoid_3parameter(z, *popts[1]))]
        # print(f'input {p0}\noptimal {list(popt)}\nerrors {perr}')
    ...
    def plot_detection_rate(self, show_fig=True, print_progress=True, parallel=True, recursed=False):
        ...
        try:
            ...
            axs[1].plot(self.zavg_efflo_effhi[:,0], self.zavg_efflo_effhi[:,1], 'o', color=colour, label=fr'$\rho$ > {SNR_THRESHOLD_LO}')
            axs[1].plot(self.zavg_efflo_effhi[:,0], self.zavg_efflo_effhi[:,2], 's', color=colour, label=fr'$\rho$ > {SNR_THRESHOLD_HI}')
            axs[1].semilogx(zaxis_plot, self.det_eff_fits[0](zaxis_plot), '-',  color=colour)
            axs[1].semilogx(zaxis_plot, self.det_eff_fits[1](zaxis_plot), '--', color=colour)
            ...
            axs[2].loglog(zaxis_plot, parallel_map(self.det_rate_limit, zaxis_plot, parallel=parallel), color='black', linewidth=1)
            axs[2].loglog(zaxis_plot, parallel_map(lambda z : self.det_rate(z, snr_threshold=10),  zaxis_plot, parallel=parallel), '-',  color=colour)
            axs[2].loglog(zaxis_plot, parallel_map(lambda z : self.det_rate(z, snr_threshold=100), zaxis_plot, parallel=parallel), '--', color=colour)
            ...
            fig.savefig(f'plots/snr_eff_rate_vs_redshift/snr_eff_rate_vs_redshift_{self.file_tag}.pdf', bbox_inches='tight')
            if show_fig:
                plt.show(fig)
            plt.close(fig) 
        except AttributeError:
            if recursed:
                raise AttributeError('Attribute missing from InjectionResults instance that is not created by calculate_detection_rate upon recursion.')
            self.calculate_detection_rate()
            self.plot_detection_rate(show_fig=show_fig, print_progress=print_progress, parallel=parallel, recursed=True)

    def print_results(self):
        """prints out summary of results
        to-do: summarise results, currently just prints column headings and results"""
        print(r'seven (7) columns: redshift $z$, integrated SNR $\rho$, measurement errors *(fractional chirp mass $\log{\mathcal{M}_c}$, fractional luminosity distance $\log{D_L}$, symmetric mass ratio $\eta$, inclination angle $\iota$), 90%-credible sky area $\Omega_{90}$', self.results)
